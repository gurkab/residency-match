...
public class Matcher {
    ...
    public static Matcher run(Map<String, List<String>> applicantsMap, Map<String, ProgramAttributes> programsMap) {
        List<Applicant> applicants = applicantsMap.entrySet().stream()
                .map(entry -> new Applicant(entry.getKey(), entry.getValue()))
                .collect(Collectors.toList());

        Map<String, Program> programs = programsMap.entrySet().stream()
    ...
    public Matcher run() {
        ...
        while (!remainingApplicants().isEmpty()) {
            ...
            System.out.print(remainingApplicants().stream()
                    .map(Applicant::getName)
                    .collect(Collectors.joining(", ")));
            System.out.println(")");

            remainingApplicants().forEach(applicant -> {
    ...
    public String resultsDescription() {
        ...
        String programDescriptions = programs.values().stream()
                .map(Program::description)
                .collect(Collectors.joining("\n"));
        str.append(programDescriptions);
        String unmatchedApplicants = applicants.stream()
